...
public class CustomShulkerBoxBlockEntity extends LootableContainerBlockEntity implements SidedInventory
{
    ...
    private CustomShulkerBoxBlockEntity.AnimationStage animationStage = CustomShulkerBoxBlockEntity.AnimationStage.CLOSED;
    ...
    {
        return ShulkerEntity.calculateBoundingBox(1.0F, state.get(CustomShulkerBoxBlock.FACING), 0.5F * this.getAnimationProgress(1.0F));
    }

    private void pushEntities(World world, BlockPos pos, BlockState state)
    {
        if (state.getBlock() instanceof CustomShulkerBoxBlock)
        {
            Direction direction = state.get(CustomShulkerBoxBlock.FACING);
    ...
    {
        return !(Block.getBlockFromItem(stack.getItem()) instanceof CustomShulkerBoxBlock);
    }
